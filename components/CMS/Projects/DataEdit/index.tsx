...
export default function EditBlog() {
    ...
    !initialised && request({ method: 'GET', url: `/api/project/data?slug=${slug}` }).then(
        res => {
            if (res?.data) {
                slug && setInitialValues(res.data?.project);
    ...
    const handleFormSubmit = async (values: {}) => {
        const { data } = await request({ method: 'POST', url: `/api/project/${slug ? 'edit' : 'create'}`, body: values });

        if (data) {
            console.log('project has been created', data);
            window.location.replace('/cms/projects')
        }
    };
                            ...
                            <Box sx={{
                                    ...
                                    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, width: '100%' }}>
                                        ...
                                        <Box sx={{
                                            ...
                                            <Box sx={{ mb: 1 }}>
                                                ...
                                                {/* Textfield */}
                                                <UiTextField placeholder={'Project summary title'} name='summaryTitle' />
                                            </Box>
                                            ...
                                            <Box sx={{ mb: 1 }}>
                                                ...
                                                {/* Textfield */}
                                                <UiTextArea placeholder={'Project summary details'} name='introduction' />
                                            </Box>
                                ...
                                <CollapseContainer title="Content Section">
                                    <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                                        <FieldLabel label={'Project Content'} />
                                        <Editor
